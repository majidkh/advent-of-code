...
pub fn part1() -> usize
{
    let junctions = get_junctions();

    // Set number of connections based on sample data or puzzle input
...
pub fn part2() -> i64
{
    ...
    let pairs = get_pairs();
    let mut circuits :Vec<Vec<usize>> = Vec::new();
    let mut last_wire_length: i64 = 0;
    ...
    'outer: for pair in pairs.iter()
    {
        ...
        if circuits.len() == 1 {
            ...
            {
                last_wire_length = junctions[pair.0.0].0 * junctions[pair.0.1].0;
                break;
            }
        }
    }
...
fn get_pairs() -> Vec<((usize, usize), f64)>
{
    // Extract junctions
    let  junctions = get_junctions();
...
fn get_junctions() -> Vec<(i64,i64,i64)>
{
    // Extract junctions
